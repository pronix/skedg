ActionController::Routing::Routes.draw do |map|
  ...
  map.namespace :provider do |provider|
    provider.login '/', :controller => :provider_sessions, :action => :new 
    provider.logout '/logout', :controller => :provider_sessions, :action => :destroy
    provider.registration '/registration', :controller => :service_providers, :action => :new
    provider.resource :provider_sessions, :only => [:new, :create, :destroy]
